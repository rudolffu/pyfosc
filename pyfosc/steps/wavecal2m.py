...
_ROOT = os.environ.get('PYFOSC_DATA_DIR') or os.path.join(os.path.dirname(sys.argv[0]), '..')
myonedstds = os.path.abspath(os.path.join(_ROOT, 'iraf_data/onedstds'))
with open('myfosc.json') as file:
    settings = json.loads(file.read())
teles = settings['mysettings']['telescope']
if teles == "XLT":
    print("Settings for XLT will be used.")
    extinct = os.path.join(_ROOT, 'extinction/baoextinct.dat')
    midname = 'XL'
elif teles == "LJT":
    print("Settings for LJT will be used.")
    extinct = os.path.join(_ROOT, 'extinction/LJextinct.dat')
    midname = 'LJ'
elif teles == "HCT":
